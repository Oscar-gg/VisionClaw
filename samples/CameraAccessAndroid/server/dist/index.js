...
const sessions = new Map(); // conversationId → claudeSessionId
// ── Claude CLI ──────────────────────────────────────────────────────────────
function runClaude(prompt, claudeSessionId) {
    return new Promise((resolve, reject) => {
        ...
        child.on("close", (code) => {
            ...
            if (stdout)
                console.log(`  [claude] stdout (${stdout.length} bytes): ${stdout.slice(0, 300)}${stdout.length > 300 ? "…" : ""}`);
            if (code !== 0) {
